...
{
    {
        ...
        ::phy_engine::model::VDC vdc{.V = 3.0};
        auto& nl{c.get_netlist()};

        auto [R1, R1_pos]{add_model(nl, r)};
        auto [R2, R2_pos]{add_model(nl, r)};
        auto [VDC, VDC_pos]{add_model(nl, vdc)};
        ...
        ::fast_io::io::println("R1: VA=",
                               r1_pin_view.pins[0].nodes->node_information.an.voltage,
                               ", VB=",
                               r1_pin_view.pins[1].nodes->node_information.an.voltage);

        auto const r2_pin_view{R2->ptr->generate_pin_view()};
        ::fast_io::io::println("R2: VA=",
                               r2_pin_view.pins[0].nodes->node_information.an.voltage,
                               ", VB=",
                               r2_pin_view.pins[1].nodes->node_information.an.voltage);
