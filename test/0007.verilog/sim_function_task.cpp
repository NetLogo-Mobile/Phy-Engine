...
module top(input a, input b, output y_func, output reg y_task);
  ...
  always @* begin
    ...
    auto& ny_func{create_node(nl)};
    auto& ny_task{create_node(nl)};
    ...
    add_to_node(nl, *vmod, 2, ny_func);
    add_to_node(nl, *vmod, 3, ny_task);
    ...
    c.digital_clk();
    if(ny_func.node_information.dn.state != ::phy_engine::model::digital_node_statement_t::false_state) { return 1; }
    if(ny_task.node_information.dn.state != ::phy_engine::model::digital_node_statement_t::true_state) { return 1; }
    ...
    set_in(inb, ::phy_engine::model::digital_node_statement_t::true_state);
    c.digital_clk();
    if(ny_func.node_information.dn.state != ::phy_engine::model::digital_node_statement_t::true_state) { return 1; }
    if(ny_task.node_information.dn.state != ::phy_engine::model::digital_node_statement_t::true_state) { return 1; }
