...
- `write_back_to_pl(experiment&, sample const&, write_back_options const&)`：可显式指定 `Logic Output` 的 `L/H/X/Z` 写回数值映射
- `write_back_now_to_pl(experiment&)` / `write_back_now_to_pl(experiment&, write_back_options const&)`：先采样当前 PE 状态，再立即写回 `.sav`
