...
inline constexpr ::std::uint_least32_t calculate_crc32_byte_common(::std::uint_least32_t crc, ::std::byte i,
															  ::std::uint_least32_t const *crctb) noexcept
{
	crc = crctb[(static_cast<::std::uint_least32_t>(i) ^ crc) & 0xff] ^ (crc >> 8);
	return crc;
...
template <crc32_option opt>
inline constexpr ::std::uint_least32_t calculate_crc32_byte(::std::uint_least32_t crc, ::std::byte i) noexcept
{
	if constexpr (opt == crc32_option::crc32)
	{
		return calculate_crc32_byte_common(crc, i, crc32_tb);
	}
	else
	{
		return calculate_crc32_byte_common(crc, i, crc32c_tb);
	}
}
template <crc32_option opt>
...
public:
	...
	inline constexpr void update_byte(::std::byte b) noexcept
	{
		crcv = calculate_crc32_byte<opt>(crcv, b);
	}
