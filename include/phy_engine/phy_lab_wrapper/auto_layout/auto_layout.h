...
struct context
{
    ...
    context(experiment& ex_, bounds2d bounds_, std::size_t w_, std::size_t h_)
        : ex(ex_), bounds(bounds_), w(w_), h(h_), occ(w_, h_)
    {
    }
};
...
inline context build_context(experiment& ex, position corner0, position corner1, options const& opt)
{
    ...
    {
        ctx.id_to_index.emplace(ex.elements()[i].identifier(), i);
    }
...
inline stats layout_cpu_fast(experiment& ex, position corner0, position corner1, double z_fixed, options const& opt)
{
    ...
    std::stable_sort(ctx.movable.begin(), ctx.movable.end(), [&](std::size_t a, std::size_t b) {
    ...
    for (auto idx : ctx.movable)
    {
        ...
        auto chosen = choose_cell(ctx.occ,
                                  ideal,
                                  fp,
                                  ctx.adj[idx],
                                  ctx.placed,
...
inline void multiply_norm_adj(weighted_adj const& g,
                              std::vector<double> const& deg,
...
inline std::vector<std::vector<double>> top_eigenvectors(weighted_adj const& g,
                                                         std::size_t k,
                                                         std::size_t iters,
                                                         std::uint32_t seed)
{
...
inline bool is_input_like(std::string_view model_id) noexcept
{
    return model_id == "Logic Input" || model_id == "8bit Input";
}

inline bool is_output_like(std::string_view model_id) noexcept
{
    return model_id == "Logic Output" || model_id == "8bit Display";
}
...
inline stats layout_cpu_hierarchical(experiment& ex, position corner0, position corner1, double z_fixed, options const& opt)
{
    ...
    for (auto idx : ctx.movable)
    {
        layers[static_cast<std::size_t>(level[idx])].push_back(idx);
    }

    auto degree = [&](std::size_t idx) -> std::size_t { return ctx.adj[idx].size(); };
    for (auto& layer_nodes : layers)
    {
        std::stable_sort(layer_nodes.begin(), layer_nodes.end(), [&](std::size_t a, std::size_t b) {
            auto const& ea = ex.elements()[a];
            auto const& eb = ex.elements()[b];
            if (ea.is_big_element() != eb.is_big_element()) return ea.is_big_element() > eb.is_big_element();
            auto const da = degree(a);
...
inline stats layout_cpu_force(experiment& ex, position corner0, position corner1, double z_fixed, options const& opt)
{
    ...
    for (std::size_t si{}; si < ctx.movable.size(); ++si)
    {
        ...
        for (auto const nb : ctx.adj[oi])
        {
            ...
            if (static_cast<std::size_t>(sj) == si) continue;
            g[si].push_back(weighted_edge{static_cast<std::size_t>(sj), 1.0});
            if (si < static_cast<std::size_t>(sj))
            {
                edges.push_back(edge_pair{si, static_cast<std::size_t>(sj), 1.0});
            }
        }
    }
    ...
    for (std::size_t it{}; it < iters; ++it)
    {
        ...
        {
            buckets[bin_idx(x[i], y[i])].push_back(i);
        }
...
inline stats layout_cpu_cluster(experiment& ex, position corner0, position corner1, double z_fixed, options const& opt)
{
    ...
    for (std::size_t si{}; si < m; ++si)
    {
        clusters[cluster_of[si]].push_back(si);
    }
    ...
    if (opt.cluster_max_nodes > 0)
    {
        ...
        for (auto const& c : clusters)
        {
            ...
            for (std::size_t i{}; i < bfs_order.size(); i += opt.cluster_max_nodes)
            {
                ...
                split.emplace_back(bfs_order.begin() + static_cast<std::ptrdiff_t>(i),
                                   bfs_order.begin() + static_cast<std::ptrdiff_t>(end));
            }
        }
        clusters.swap(split);
    ...
    std::stable_sort(cluster_order.begin(), cluster_order.end(), [&](std::size_t a, std::size_t b) {
    ...
    auto choose_macro = [&](std::size_t cid) -> std::optional<cell> {
        ...
        auto center = [](cell c, footprint f) -> std::pair<double, double> {
            return {static_cast<double>(c.x) + 0.5 * static_cast<double>(f.w),
                    static_cast<double>(c.y) + 0.5 * static_cast<double>(f.h)};
        };
        ...
        auto consider = [&](cell c) {
            ...
            double cost = 0.0;
            cost += opt.cluster_macro_ideal_weight *
                    static_cast<double>(std::abs(c.x - ideal.x) + std::abs(c.y - ideal.y));
    ...
    for (auto cid : cluster_order)
    {
        ...
        for (auto si : nodes)
        {
            ...
            auto chosen = choose_cell(occ_elem,
                                      ideal,
                                      fp,
                                      ctx.adj[idx],
                                      ctx.placed,
